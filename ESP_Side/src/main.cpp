      ...
      if (client.available()) {
        ...
        if (c == '\n') {
          ...
          if (currentline.length() == 0) {
            ...
            if (body.substring(10) != "") {
              ...
              for (int i = 0; i < msg.length(); i++) {
                if (Baudot.isLetter(msg[i]) && Baudot.getMode() != 1) {
                  ...
                  EncodedMsg[arrpos] = Baudot.Encode(msg[i]);
                  if (msg[i] == '!')
                    EncodedMsg[arrpos] = 13;
                  if (msg[i] == '$')
                    EncodedMsg[arrpos] = 9;
                  if (msg[i] == '"')
                    EncodedMsg[arrpos] = 17;
                  if (msg[i] == ';')
                    EncodedMsg[arrpos] = 30;
                  if (msg[i] == '#')
                    EncodedMsg[arrpos] = 20;
                  if (msg[i] == '&')
                    EncodedMsg[arrpos] = 26;
                  arrpos++;
                } else {
                  EncodedMsg[arrpos] = Baudot.Encode(msg[i]);
                  if (msg[i] == '!')
              ...
              for (int i = 0; i < 10; i++) {
                if (Baudot.isLetter(checkstr[i]) && Baudot.getMode() != 1) {
                  ...
                  arrpos++;
                } else if (!Baudot.isLetter(checkstr[i]) &&
                           Baudot.getMode() != 0) {
                  Baudot.setMode(0);
                  EncodedMsg[arrpos] = BAUDOT_FIGURES;
                  arrpos++;
              ...
              sent_text = sent_text + "Sending: " + content +
                          "<br>Full Message: " + msg + "<br>";
            ...
            client.println("<style>input {width: 100%;padding: 0px;border: 1px "
                           "solid black;margin-bottom: 2px;}");
            client.println("div {display: flex;}");
            client.println("p {margin: 0px;padding: 0px;border: 1px solid "
                           "black;overflow: auto}");
            ...
            client.println("<form method = \"post\"><input name = "
                           "\"send_data\" type = \"text\" placeholder=\"Enter "
                           "text to send\"><input type = \"submit\"></form>");
